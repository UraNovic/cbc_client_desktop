...
describe('send', function() {
  ...
  before(function(){
    ...
    // Login the user
    browser.executeScript('store.set("cbc_auth",{' +
  ...
  describe('recipient', function() {
    ...
    it('should not accept an invalid cbc address', function(done) {
      input.sendKeys('invalidcbcaddress');
      expect(input.getAttribute('class'))
        .to.eventually.contain('ng-invalid')
        .and.notify(done);
    });

    it('should accept a valid cbc address', function(done) {
  ...
  describe('send cbc button', function() {

    var input = $('#sendcbcButton');
