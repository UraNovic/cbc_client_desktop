...
SendTab.prototype.angular = function (module)
{
                                           ...
                                           $network, keychain)
  {
    ...
    $scope.check_destination = function () {
      ...
      if (!cbcAddressCodec.isValidAddress(recipient)) return;

      if (!$scope.onlineMode) {
        $scope.send.currency = '';
        $scope.send.path_status  = "none";
        $scope.send.currency_choices = ['cbc'];
    ...
    $scope.update_currency = function () {
      ...
      if (!cbcAddressCodec.isValidAddress(recipient) && !send.quote_url) {
        return;
      }
    ...
    $scope.update_amount = function () {
      ...
      if (send.quote_url) {
        ...
        if (!cbcAddressCodec.isValidAddress(recipient) || !cbc.Amount.is_valid(amount)) {
          // XXX Error?
          return;
        }
