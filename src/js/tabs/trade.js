                                            ...
                                            keychain, $rootScope)
  {
    ...
    function update_pairs() {
      ...
      if (!settings.hasSetting($scope.userBlob, 'trade_currency_pairs')) {
        $scope.pairs_query = [{name: 'cbc/USD.rMwjYedjc7qqtKYVLiAccJSmCwih4LnE2q' },
          { name: 'cbc/USD.rvYAfWj5gh67oV6fW32ZzP3Aw4Eubs59B' },
          { name: 'cbc/JPY.r94s8px6kSw1uZ1MV98dhSRTvc6VMPoPcN' },
          { name: 'BTC.rvYAfWj5gh67oV6fW32ZzP3Aw4Eubs59B/cbc' },
          { name: 'BTC.rMwjYedjc7qqtKYVLiAccJSmCwih4LnE2q/cbc' }];
      }
      else {
        $scope.pairs_query = settings.getSetting($scope.userBlob, 'trade_currency_pairs');
      }
    ...
    $scope.reset = function () {
      ...
      var pair = store.get('cbc_currency_pair') || $scope.pairs_all[0].name;
      ...
      else if(store.get('cbc_listing')) {
        listing = store.get('cbc_listing');
      }
      else {
        listing = 'orderbook';
      }
    ...
    */
    ...
    $scope.update_first = function (type) {
      ...
      order.first_amount = cbc.Amount.from_human(formatted, {reference_date: new Date(+new Date() + 5*60000)});
    ...
    $scope.update_price = function (type) {
      ...
      order.price_amount = cbc.Amount.from_human(formatted, {reference_date: new Date(+new Date() + 5*60000)});
    ...
    $scope.update_second = function (type) {
      ...
      order.second_amount = cbc.Amount.from_human(formatted, {reference_date: new Date(+new Date() + 5*60000)});
    ...
    function updateSettings() {
      ...
      if (!store.disabled) {
        store.set('cbc_currency_pair', pair);
      }
      ...
      if ((!first_currency.is_native() && !cbcAddressCodec.isValidAddress(order.first_issuer)) ||
          (!second_currency.is_native() && !cbcAddressCodec.isValidAddress(order.second_issuer)) ||
    ...
    $scope.add_pair = function() {
      ...
      // if (!data.clients || !data.clients.cbctradecom ||
      //     !data.clients.cbctradecom.trade_currency_pairs ||
      //     !angular.isArray(data.clients.cbctradecom.trade_currency_pairs)) {
      // } else {
    ...
    if ($routeParams.first && $routeParams.second) {
      ...
      ['first','second'].forEach(function(prefix){
        ...
        if (routeIssuers[prefix]) {
          if (cbcAddressCodec.isValidAddress(routeIssuers[prefix][1])) {
