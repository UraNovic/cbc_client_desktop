...
TrustTab.prototype.angular = function (module) {
  module.controller('TrustCtrl', ['$scope', 'rpBooks', '$timeout',
    ...
    function ($scope, books, $timeout, $routeParams, id, $filter,
      ...
      var RemoteFlagDefaultcbc = 0x00800000;
    ...
    $scope.saveTransaction = function(tx) {
      ...
      $scope.$watch('account', function() {
        $scope.acctDefaultcbcFlag = ($scope.account.Flags & RemoteFlagDefaultcbc);
      ...
      $scope.grant = function() {
        ...
        $scope.$apply(function() {
          ...
          var amount = cbc.Amount.from_human('' + $scope.amount + ' ' + $scope.lineCurrencyObj.to_hex(), {reference_date: new Date(+new Date() + 5 * 60000)});
      ...
      $scope.grant_confirmed = function() {
        ...
        // Nocbc flag
        if ($scope.ripplingFlag === 'tfClearNocbc') {
          flags.push('ClearNocbc');
        } else if ($scope.ripplingFlag === 'tfSetNocbc') {
          flags.push('SetNocbc');
        }
  ...
  module.controller('AccountRowCtrl', ['$scope', 'rpBooks', 'rpNetwork', 'rpId', 'rpKeychain', '$timeout',
    function ($scope, books, $network, id, keychain, $timeout) {
      ...
      $scope.save_account = function () {
        ...
        var flags = [];
        // Nocbc flag
        if ($scope.trust.ripplingFlag === 'tfClearNocbc') {
          flags.push('ClearNocbc');
        } else if ($scope.trust.ripplingFlag === 'tfSetNocbc') {
          flags.push('SetNocbc');
        }
        // Auth flag
