...
ColdWalletTab.prototype.angular = function (module) {
  ...
  function ($scope, $routeParams, $location, $route, id, network) {
    ...
    // Parse the transaction returned by cbc-lib
    ...
    /*function formatTxnMessage(txn, address) {
      ...
      txnMessage += ' The fee was ' + txn.outcome.fee + ' cbc. ' +
    ...
    var watcher = $scope.$watch('connected', function() {
      ...
      account.entry(function(err, entry) {
        ...
        var defaultcbc = !!(entry.account_data.Flags & cbc.Remote.flags.account_root.Defaultcbc);
        var requireAuth = !!(entry.account_data.Flags & cbc.Remote.flags.account_root.RequireAuth);
        var globalFreeze = !!(entry.account_data.Flags & cbc.Remote.flags.account_root.GlobalFreeze);
        ...
        accountInfo.push({
          setting: 'Default cbc',
          enabled: defaultcbc,
          description: 'Allow balances in trust lines to cbc by default'
        });
        accountInfo.push({
        ...
        network.remote.requestAccountInfo({account: address})
          .on('success', function(info) {
            $scope.$apply(function() {
              ...
              $scope.cbcBalance = info.account_data.Balance / 1000000;

              // To calcute the cbc reserve and available balance
