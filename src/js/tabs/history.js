...
HistoryTab.prototype.angular = function (module) {
                                     ...
                                     function ($scope, $id, $network, filedialog)
  {
    ...
    if (store.get('cbc_history_type_selections')) {
      $scope.types = $.extend(true,$scope.types,store.get('cbc_history_type_selections'));
    }

    // Filters
    if (store.get('cbc_history_filters')) {
      $scope.filters = store.get('cbc_history_filters');
    } else {
      $scope.filters = {
        ...
        .on('success', function(data) {
          if (data.transactions.length) {
            for(var i=0;i<data.transactions.length;i++) {
              var date = cbc.utils.toTimestamp(data.transactions[i].tx.date);
    ...
    $scope.$watch('types', function(){
      ...
      if (!store.disabled) {
        store.set('cbc_history_type_selections', checked);
      }
    }, true);

    if (!store.disabled) {
      $scope.$watch('filters', function(){
        store.set('cbc_history_filters', $scope.filters);
      }, true);
    }
      ...
      if (history.length) {
        ...
        history.forEach(function(event)
        {
          ...
          var affectedCurrencies = _.map(event.affected_currencies, function (currencyCode) {
            return cbc.Currency.from_json(currencyCode).to_human();
          });
        ...
        $scope.$apply(function () {
          ...
          if (data.transactions) {
            ...
            data.transactions.forEach(function (e) {
              ...
              if (tx) {
                var date = cbc.utils.toTimestamp(tx.date);
    ...
    function prepareCsv(cb) {
      ...
      var cbcIssuer = 'rrrrrrrrrrrrrrrrrrrrrhoLvTp';
      ...
      function getOrderDetails(keyVal, buy, sell) {
        if (buy !== null && buy instanceof Amount) {
          ...
          keyVal.SentIssuer = buy.issuer() === cbcIssuer ? 'NA' : buy.issuer();
        }

        if (sell !== null && sell instanceof Amount) {
          keyVal.RecvAmount = formatAmount(sell);
          keyVal.RecvCcy = sell.currency().get_iso();
          keyVal.RecvIssuer = sell.issuer() === cbcIssuer ? 'NA' : sell.issuer();
        }
      }
        ...
        if (type === 'TrustSet') {
          ...
          if (exists(transaction.amountSent)) {
            ...
            if (exists(transaction.sendMax)) {
              linePayment.SentIssuer = transaction.sendMax.issuer === cbcIssuer ? 'NA' : transaction.sendMax.issuer;
            }
          }

          linePayment.RecvAmount = formatAmount(transaction.amount);
          linePayment.RecvCcy = transaction.currency;
          linePayment.RecvIssuer = transaction.amount.issuer() === cbcIssuer ? 'NA' : transaction.amount.issuer();
