...
var rewriter = require('../util/jsonrewriter'),
  ...
  Amount = cbc.Amount,
  cbcAddress = require('../util/types').cbcAddress,
  fs = require('fs');
                                        ...
                                        keychain, $route, $timeout, fileDialog)
{
  ...
  function handleAccountLoad(e, data)
  {
    ...
    remote.requestAccountInfo({
      ...
      if (accountError) {
        ...
        var regularKeyPublic = new cbcAddress($scope.userBlob.data.regularKey).getAddress();
    ...
    $scope.$apply(function () {
      ...
      for (var n=0, l=data.lines.length; n<l; n++) {
        ...
        line = $.extend({}, line, {
          ...
          limit_peer: cbc.Amount.from_json({value: line.limit_peer, currency: line.currency, issuer: account}),
          balance: cbc.Amount.from_json({value: line.balance, currency: line.currency, issuer: account})
        });
  ...
  var storeCurrenciesAll = store.get('cbc_currencies_all') || [];
