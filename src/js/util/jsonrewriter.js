  ...
  processTxn: function (tx, meta, account) {
    try {
      ...
      if (tx && 'object' === typeof tx) {
        ...
        transaction.dateRaw = cbc.utils.fromTimestamp(transaction.date);
      }
    ...
    if (tx.Account === account
      ...
      if ('tesSUCCESS' === meta.TransactionResult) {
        switch (tx.TransactionType) {
          case 'Payment':
            var amount = cbc.Amount.from_json(tx.Amount);
    ...
    if ('tesSUCCESS' === meta.TransactionResult) {
      meta.AffectedNodes.forEach(function (n) {
        ...
        if (node.entryType === "cbcState"
          ...
          var nocbcFlag = high.issuer === account ? 'HighNocbc' : 'LowNocbc';
          var freezeFlag = high.issuer === account ? 'HighFreeze' : 'LowFreeze';
          var authFlag = high.issuer === account ? 'HighAuth' : 'LowAuth';

          // New trust line
          if (node.diffType === "CreatedNode") {
            effect.limit = cbc.Amount.from_json(high.value > 0 ? high : low);
            effect.limit_peer = cbc.Amount.from_json(high.value > 0 ? low : high);
          ...
          // Modified trust line
            ...
            // Trust flag change (effect gets this type only if nothing else but flags has been changed)
              ...
              if (
                ...
                (node.fields.Flags & cbc.Remote.flags.state[nocbcFlag] &&
                  !(node.fieldsPrev.Flags & cbc.Remote.flags.state[nocbcFlag]))
                ||
                ...
                (node.fields.Flags & cbc.Remote.flags.state[freezeFlag] &&
                  !(node.fieldsPrev.Flags & cbc.Remote.flags.state[freezeFlag]))
                ||
                ...
                (node.fields.Flags & cbc.Remote.flags.state[authFlag] &&
                  !(node.fieldsPrev.Flags & cbc.Remote.flags.state[authFlag]))
              )
              {
          ...
          if (!$.isEmptyObject(effect)) {
            ...
            // nocbc flag
            if (node.fields.Flags & cbc.Remote.flags.state[nocbcFlag]) {
              effect.nocbc = true;
            }

            // freeze flag
            if (node.fields.Flags & cbc.Remote.flags.state[freezeFlag]) {
              effect.freeze = true;
            }

            // auth flag
            if (node.fields.Flags & cbc.Remote.flags.state[authFlag]) {
              effect.auth = true;
            }
          }
        }
